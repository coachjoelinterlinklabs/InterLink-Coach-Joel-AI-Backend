/* -------------------------------------------------------
  Coach Joel AI — Browser-Only Version (ChatGPT-style layout)
  InterLink Community Role-Integrated Version
------------------------------------------------------- */
...
- When listing steps or winners, use numbered lists (1., 2., 3.) or emoji bullets.
- Use spacing and margins for clarity without asterisks.
- End with a brief encouraging sentence.
- If sending a link, analyze whether it is official from InterLink. If uncertain, ask the user if they want to explore it and offer your advice.

Constraints:
1. Use multiple languages if the user requests translation.
2. Stay focused on InterLink Labs Projects. You may give feedback using the InterLink Coach House Points System.
3. If information is missing, ask a clarifying question.
4. If outside your scope, politely decline and refer to official InterLink resources.
...
function appendMessage(role, text) {
  ...
  div.classList.add("flex", "w-full", "my-3");
  ...
  if (role === "user") {
    ...
    bubble.classList.add("bg-[#6962F1]", "text-white");
    div.appendChild(bubble);
    div.appendChild(img);
  } else {
    div.classList.add("justify-start");
    bubble.classList.add("bg-[#F5F7FA]", "text-[#0E121B]");
    div.appendChild(img);
    div.appendChild(bubble);
  }
...
async function sendMessage() {
  ...
  try {
    ...
    console.error(err);
    chatOutput.lastChild.remove();
    appendMessage("ai", "⚠️ Network or API error. Please try again.");
  }
}
...
emojiBtn.addEventListener("click", (e) => {
  ...
  emojiBtn.parentElement.appendChild(emojiMenu);
});
